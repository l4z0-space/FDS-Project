...
void ClientMenu() {
	...
	if (w && w>0 && w<3) {
		...
		case 2: { Clients.push_back(new Client()); cout << "\n\n      Successfully registered\n\n";
			...
			o.open("Clients.txt", ios_base::app);
			o << line;
			o.close();
			break; }
...
void OwnerMenu() {
	...
	if (ch == "1") {
			...
			for (unsigned int u = 0; u < Owners.size(); u++) {
				...
				if (Owners[u]->getID() == pass) {
					...
					if (w) {
						switch (w) {
						case 1:
                            if (Owners[u]->getObudget() >= 1000){

                                WebEarnings += Owners[u]->getObudget()*0.05;
                                cout << "\n " << Owners[u]->getCompany()->getCname() << " successfully promoted!\n\n";

                                cout << "Web commission (5% of budget): " << Owners[u]->getObudget()*0.05 <<"\n\n";

                                cout << "Current Company budget: $" << Owners[u]->getObudget() - Owners[u]->getObudget()*0.05 << "\n\n";
						...
						case 2: int pos;
                                for (unsigned int q = 0; q < Promoted_Companies.size(); q++) {

                                    if (Promoted_Companies[q]->getCname() == Owners[u]->getCompany()->getCname()){
                                        pos = u;
                                        Promoted_Companies.erase(Promoted_Companies.begin() + (pos - 1));
                                    }break;
                                }break;
		...
		if (Owners[Owners.size() - 1]->getAccess() != Owners[Owners.size() - 1]->getCompany()->getCode()) { cout << "\nUser cannot be registered because of WRONG access code!\n\n"; Owners.pop_back(); }
			...
			line += company; line += ",";
			line += budget; line += ",";										// initializing the file
			line += Owners[Owners.size() - 1]->getID(); line += ",";
			line += Owners[Owners.size() - 1]->getAccess(); line += "\n";
			o.open("Owners.txt", ios_base::app);		// ios_base::app used to only append and not remove the existing information in a file
			o << line;
			o.close();
		}
	}
	...
	if (choice == "1") {
					...
					if (Promoted_Companies.size() == 0) { cout << setw(20) << "Apply Later\n\n"; }
						...
						if (a <= Promoted_Companies.size()) {
							...
							if (Candidates[u]->getDesiredWage() <= Promoted_Companies[a - 1]->getWage()) {		// if requirments are met
								...
								ofstream c;
								c.open("HiringSession.txt", ios_base::app);
		...
		if (o == "1") {
			...
			if (qs == "1") {
				...
				ofstream s;
				s.open("Candidates.txt", ios_base::app);
				s << expE;
				s.close();
			...
			c.open("Candidates.txt", ios_base::app);
			string line = "intern,";
...
void readCompanies() {
	...
	ifstream Database("Available.txt");
...
void readOwners() {
	...
	ifstream O("Owners.txt");

	if (O.fail()) { cout << "\nProblem importing Owners data!\n"; }
...
void readCandidates() {
	...
	ifstream C;
	C.open("Candidates.txt");

	if (C.fail()) { cout << "\nFailed accessing Candidates!\n"; }
